...
def collect_grasps(cfg : DictConfig) -> None:
    ...
    if cfg.collector_config.type in ["cio", "grad"]:
        ...
        target_num = 100
        if os.path.exists(f'/apdcephfs/private_qihangfang/ibsdatasetgraspdtest/{cfg.collector_config.name}'):
            a = np.loadtxt(f'/apdcephfs/private_qihangfang/ibsdatasetgraspdtest/{cfg.collector_config.name}')
            if len(a.shape) == 1:
                target_num = 99
            else:
        ...
        f = open('/apdcephfs/private_qihangfang/finish_gene2', 'a')
