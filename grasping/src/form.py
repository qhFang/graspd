...
class FORM():
    ...
    def run(self, initial_guess):
        ...
        if self.coarse_to_fine:
            level_sets[n_pose_only:n-1000] = torch.linspace(self.backoff.item(), 0.0, n-1000-n_pose_only)
        ...
        for i in range(n):
            ...
            G = state.contact_matrix[palm_idx].permute(1, 2, 0).reshape(1, 6, -1)            
            Gt = G.transpose(1,2)
            temp = torch.tensor(0.01).float().to(self.device) * torch.tensor(np.eye(6)).float().to(self.device)
            temp = torch.matmul(G, Gt) - temp
            eigval = torch.linalg.eigh(temp.cpu())[0].to(self.device)
            rnev = F.relu(-eigval)
            l_rank = torch.sum(rnev * rnev, 1).squeeze(0)
            
            l_netwrench = F.relu(torch.sum(torch.sum(state.contact_f_s[self.box_contact_inds][palm_idx]*state.contact_f_s[self.box_contact_inds][palm_idx], 0)))     
            
            l_distance = state.contact_world_dist[self.box_contact_inds][palm_idx] * state.contact_world_dist[self.box_contact_inds][palm_idx]
            
            l_penetration = F.relu(-state.contact_world_dist[self.box_contact_inds]) * 10  # B x V
            
            
            l_all = l_rank + l_netwrench + l_distance.sum(0) + l_penetration.sum(0) + l_normals
            ...
            if i % record_every == 0:
                ...
                print(f"{i}/{n} task:{l_all} distance:{l_distance} normal: {l_normals} netwrench:{l_netwrench} rank: {l_rank} {torch.linalg.matrix_rank(G)}")
                print(state.contact_world_dist[self.box_contact_inds][palm_idx])
                #if i == 200:
                #    exit()
                ...
                history['l_distance'][i // record_every+1] = l_distance.sum(0).detach().cpu()
        ...
        results = {
            ...
            'final_l_distance': l_distance.sum(0).detach().cpu(),
