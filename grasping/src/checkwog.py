...
#[322, 71, 547],[322, 547, 741],[322, 741, 689],[322, 689, 567],[322, 567, 333],[322, 333, 703],[322, 703, 164],[322, 164, 513],[322, 513, 110],[322, 110, 379],[322, 379, 31],[322, 31, 500],[322, 500, 480],[322, 480, 605]
...
class REF():
    ...
    def physical_test(self, hand_q, mano_q, mu=0, contact_kf=1e8):
        ...
        for i in range(len(test_gravities)):
            ...
            self.model.gravity = torch.zeros(3, device=self.device)
            #print(test_gravity, test_gravity.norm())
    ...
    def run(self, initial_guess):
        ...
        refs = np.loadtxt(f'/apdcephfs/share_1330077/qihangfang/network/rlisolocaltrain/{self.obj_name}.txt')[:100]
        ...
        for ii in range(100):
            ...
            ii = 40
            ref = refs[ii]
            hand_q, mano_q = torch.from_numpy(ref[:7]).float().cuda(), torch.from_numpy(ref[7:]).float().cuda()
            mano_q[3:] = 0
            #print(mano_q)
            ...
            net_wrench = torch.matmul(state.contact_matrix[self.box_contact_inds][palm_idx], state.contact_world_n[self.box_contact_inds][palm_idx].unsqueeze(-1))        
            ...
            lr_hand_q = 2e-3 # 2e-4 worked well
            lr_mano_q = 1e-2 # 3e-3 worked well
            ...
            #try:
            #    os.makedirs(f'/apdcephfs/share_1330077/qihangfang/network2/rlisolocaltrainbefore/{self.obj_name}')
            #except OSError:
            #    pass


            #path = os.path.join(f'/apdcephfs/share_1330077/qihangfang/network2/rlisolocaltrainbefore/{self.obj_name}/post_{ii}.obj')                
            #path = '/apdcephfs/private_qihangfang/111415.obj'
            #handmesh = trimesh.Trimesh(contact_world_pos.detach().cpu().numpy(), np.concatenate([self.mano_layer.th_faces.detach().cpu().numpy(), new_faces],axis=0), process=False)
            #handmesh.export(path)

            
            for optim in range(200):
                contact_point0_before = torch.clone(self.model.contact_point0)
                mano_output = self.mano_layer(mano_q[None,:], self.mano_shape)
                vertices = mano_output.verts
                vertices *= self.mano_ratio
                ...
                l_distance_finger = 0
                for i in range(len(finger_idx)):
                    l_distance_finger += state.contact_world_dist[self.box_contact_inds][finger_idx[i]].abs().mean()
                ...
                l_penetration = torch.sum(F.relu(-state.contact_world_dist[self.box_contact_inds]))
                self.pene_list.append(state.contact_world_dist[self.box_contact_inds].min().item())
                l_all = 3 * l_distance + l_penetration + l_distance_finger / 5
                hand_q_optimizer.zero_grad()
                l_all.backward()
                hand_q_optimizer.step()

                path = os.path.join(f'/apdcephfs/share_1330077/qihangfang/videos/{optim + 5}.obj')                
                print(path)
            ...
            for i in range(1):
                for optim in range(50):
                    ...
                    state = self.model.state()
                    state.joint_q[self.inds] = hand_q[:self.n_inds].detach()
                    state.joint_q[self.inds[0:3]] = self.pos_ratio*hand_q[0:3].detach()
                    #state.joint_q[self.inds[3:7]] = self.quat_ratio * hand_q[3:7]
                    ...
                    l_distance_finger = state.contact_world_dist[self.box_contact_inds][finger_idx[i]].abs().mean()
                    print(l_distance_finger)
                    
                    l_all = l_distance_finger
                    hand_q_optimizer.zero_grad()
                    l_all.backward()
                    hand_q_optimizer.step()
            '''
            ...
            try:
                os.makedirs(f'/apdcephfs/share_1330077/qihangfang/network2/rlisolocaltrain/{self.obj_name}')
            except OSError:
                pass


            path = os.path.join(f'/apdcephfs/share_1330077/qihangfang/network2/rlisolocaltrain/{self.obj_name}/post_{ii}.obj')                
            handmesh = trimesh.Trimesh(contact_world_pos.detach().cpu().numpy(), np.concatenate([self.mano_layer.th_faces.detach().cpu().numpy(), new_faces],axis=0))
            handmesh.export(path)
            ...
            #print('------------')
        f = open('/apdcephfs/share_1330077/qihangfang/baselines/results_rlisolocal', 'a')
        ...
        f.write(out_str)
        f.close()
        np.save(f'/apdcephfs/share_1330077/qihangfang/network2/rlisolocal/{self.obj_name}_pene.txt', np.array(self.pene_list))
