...
class REF():
    ...
    def run(self, initial_guess):
        ...
        refs = np.loadtxt(f'/apdcephfs/private_qihangfang/ibsdatasetgraspdtest/{self.obj_name}')[:100, 1:-2]
        ...
        start_pose = np.loadtxt(f'/apdcephfs/private_qihangfang/ibsdatasetstart/{self.obj_name}').reshape(100, -1)[0]
        hand_q0 = torch.from_numpy(start_pose[1:8]).float().to('cuda')
        mano_q0 = torch.from_numpy(start_pose[8:]).float().to('cuda')
        ...
        for ii in range(len(refs)):
            ...
            for i in range(4):
                hand_q_c = hand_q0.clone()
                hand_q_c[:3] = hand_q0[:3] + i * (hand_q[:3] - hand_q0[:3]) / 3
                mano_q_c = mano_q0
                mano_q_c[:3] = mano_q0[:3] + i * (mano_q[:3] - mano_q0[:3]) / 3
                if i == 3:
                    hand_q_c = hand_q
                    mano_q_c = mano_q

                contact_point0_before = torch.clone(self.model.contact_point0)
                mano_output = self.mano_layer(mano_q_c[None,:], self.mano_shape)
                vertices = mano_output.verts
                vertices *= self.mano_ratio
                ...
                self.model.contact_point0[:778,:]= vertices[0,:,:]
                state = self.model.state()
                state.joint_q[self.inds] = hand_q_c[:self.n_inds].detach()
                state.joint_q[self.inds[0:3]] = self.pos_ratio*hand_q_c[0:3]
                state.joint_q[self.inds[3:7]] = self.quat_ratio * hand_q_c[3:7]
                ...
                path = f'/apdcephfs/private_qihangfang/opprocess/graspd{i}.obj'
            ...
            try:
                os.makedirs(f'/apdcephfs/share_1330077/qihangfang/network2/graspdbefore/{self.obj_name}')
            except OSError:
                pass
            ...
            for optim in range(1):
                ...
                #print(l_distance.item(), l_penetration.item(), l_netwrench.item(), l_normals.item())
                # + 3 * l_distance_finger
                print(l_netwrench)
            try:
            ...
            if True:
                ...
                #print(simution[0])
            path = os.path.join(f'/apdcephfs/share_1330077/qihangfang/baseline/graspd/{self.obj_name}/post_{ii}.obj')                
            handmesh = trimesh.Trimesh(contact_world_pos.detach().cpu().numpy(), np.concatenate([self.mano_layer.th_faces.detach().cpu().numpy(), new_faces],axis=0))
            handmesh.export(path)

            if (l_netwrench < 1e-4) and (rank == 6):
            ...
            try:
                os.makedirs(f'/apdcephfs/share_1330077/qihangfang/baseline/graspd/{self.obj_name}')
            except OSError:
                pass
        ...
        exit()
        f = open('/apdcephfs/share_1330077/qihangfang/baselines/results_graspd', 'a')
