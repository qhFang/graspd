...
class REF():
    ...
    def run(self, initial_guess):
        ...
        #refs = np.loadtxt(f'/apdcephfs/private_qihangfang/ibsgraspingt/{self.obj_name}/{3}')
        refs = np.loadtxt(f'/apdcephfs/share_1330077/qihangfang/network2/rlshadow//{self.obj_name}_1.txt')
        ...
        for ii in range(len(refs)):
            ...
            #local_q = refs[ii].float().to(self.device)
            ...
            '''
            lr_local_q = 1e-3 # 3e-3 worked well
            ...
            for optim in range(500):
                ...
                l_all = l_distance + optim / 1000 *  l_penetration
                global_q_optimizer.zero_grad()
                l_all.backward()
                global_q_optimizer.step()
                
                # + 3 * l_distance_finger
                #print(l_distance.item(), l_penetration.item(), l_netwrench.item())
            #exit()
            ...
            try:
                os.makedirs(f'/apdcephfs/share_1330077/qihangfang/network2/rlshadow1/{self.obj_name}')
            except OSError:
                pass

            path = os.path.join(f'/apdcephfs/share_1330077/qihangfang/network2/rlshadow1/{self.obj_name}/{ii}.obj')                
            handmesh.export(path)
            penetration_points.append(get_sample_intersect_volume(self.obj_mesh, handmesh))
            ...
            #print('------------')
        f = open('/apdcephfs/share_1330077/qihangfang/baselines/results_shadow', 'a')
