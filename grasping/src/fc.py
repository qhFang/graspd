...
class FC():
    ...
    def build_model(self, obj_config, with_viewer_mesh=False):
        ...
        if self.model != None:
            ...
            for i in range(n_attempts):
                ...
                mano_input[:3] = tf.quaternion_to_axis_angle(torch.tensor(self.quat,dtype=torch.float,device=self.device)[[3,0,1,2]])
        ...
        if obj_config.primitive_type == "sphere":
            ...
            self.mesh_norm_factor = mesh_norm_factor
            self.obj_vertices = torch.from_numpy(vertices).float().cuda()
    ...
    def sample_initial_guess(self):
        ...
        hand_q[:3] = state.joint_q[:3]
        hand_q[0:3] /= self.pos_ratio
        hand_q[6] = 1.0

        quat = state.joint_q[[6,3,4,5]]
        mano_q[:3] = tf.quaternion_to_axis_angle(quat)
    ...
    def run(self, initial_guess):
        ...
        f = open(f'/apdcephfs/private_qihangfang/ibsdatasetstart/{self.obj_name}', 'a')
        f.write(f'{self.nums} ')
        for i in range(7):
            f.write(f'{hand_q[i]} ')
        ...
        if True:
            level_sets[n_pose_only:n-5000] = torch.linspace(self.backoff, 0.0, n-5000-n_pose_only)
        ...
        for i in range(n):
            ...
            vertices *= self.mano_ratio
            self.model.contact_point0[:778,:] = vertices[0,:,:]
            #print(self.model.contact_point0[778:778+len(self.vertices), :].shape, self.vertices.shape)
            #self.model.contact_point0[778:778+len(self.vertices), :] = self.vertices.detach() * 2.0
            ...
            #R = tf.axis_angle_to_matrix(mano_q[:3])
            #tr = torch.eye(4, device=self.device)
            #tr[:3, :3] = R
            #tr[:3, 3] = hand_q[:3]
            #tr = torch.linalg.inv(tr.detach().cpu()).to(self.device)
            #print(self.obj_vertices.shape)
            #smple_point = torch.cat([self.obj_vertices, torch.ones(self.obj_vertices.shape[0], 1).float().to('cuda')], -1)
            #obj_pcd = torch.matmul(tr, sample_point.transpose(0, 1)).transpose(0, 1)[:, :3]

            #print(obj_pcd.max(dim=0)[0], obj_pcd.min(dim=0)[0])
            #exit()

            #l_distance = torch.mean(state.contact_world_dist[self.box_contact_inds][palm_idx] * state.contact_world_dist[self.box_contact_inds][palm_idx])
            l_distance = torch.mean(state.contact_world_dist[self.box_contact_inds][palm_idx].abs())
            l_distance_pure = l_distance.item()
            #print(state.contact_world_dist[778:778+len(self.vertices)].abs().max())
            finger_dis = [state.contact_world_dist[self.box_contact_inds][finger_idx[i]].abs().min() for i in range(len(finger_idx))]
            l_distance_tip = torch.tensor(finger_dis).mean()

            #l_distance_tip = torch.mean(state.contact_world_dist[self.box_contact_inds][tip_idx].abs())

            l_distance = l_distance + 5 * l_distance_tip
            #print(state.contact_world_pos[self.box_contact_inds][self.mano_layer.th_faces, :].shape, self.mano_layer.th_faces.shape)
            
            #a = state.contact_world_pos.cpu().detach().numpy()
            #np.savetxt('/apdcephfs/private_qihangfang/pos.txt', a)
            #exit()
            ...
            #contact_leaky_f_s
            #net_wrench = state.contact_f_s[self.box_contact_inds][palm_idx] / 1e3
            #net_wrench = torch.sum(net_wrench, 0)
            #woleaky = torch.mean(net_wrench * net_wrench)
            ...
            l_joint = torch.norm(hand_q[middle_inds],dim=0)
            
            l_penetration = torch.sum(F.relu(-state.contact_world_dist[self.box_contact_inds]))
            
            #success check
            '''
            if (torch.linalg.matrix_rank(grasp_matrix) == 6):
                if (l_distance < 0.01) and (l_netwrench < 0.05) and (l_penetration < 0.01):
                    break
                
                if (-state.contact_world_dist[self.box_contact_inds].min() < 0.01):
                    if ((state.contact_world_dist[self.box_contact_inds][palm_idx] < 0.01).sum() >= 10):
                        if ((state.contact_world_dist[self.box_contact_inds][tip_idx] < 0.01).sum() >= 5):
                            break
                            if (l_netwrench < 0.1):
                                succ_dir = self.geometry_normal_test(hand_q, mano_q)
                                succ_flag = True
                                for k in range(len(succ_dir)):
                                    if not succ_dir[k]:
                                        succ_flag = False
                                        break
                                if (succ_flag):
            ...
            l_all = 100 * l_distance + l_normals + 100 * l_penetration + 1000 * l_rank + 100 * l_netwrench + 0.01 * mano_q.norm()
            
            
            #l_all = l_distance + 0.01 * l_normals + l_penetration + 1000 * l_rank + l_netwrench
            #grad = torch.autograd.grad(l_all, mano_q, torch.ones_like(l_all), retain_graph=True)[0]
            #print(grad)
            
            #l_all = 100 * l_distance + l_normals + 100 * l_penetration + 1000 * l_rank + 0.01 * mano_q.norm()

            #l_all = l_netwrench
            #l_all = 100 * l_distance + l_normals + 100 * l_penetration
            #l_all = 100 * l_distance + 100 * l_normals + l_penetration + 1000 * l_rank + l_netwrench + 0.01 * mano_q.norm()
            #l_all = 100 * l_distance + 100 * l_penetration + 1000 * l_rank + l_netwrench + 0.01 * mano_q.norm()
            ...
            if i % record_every == 0:
                ...
                print(f"{i}/{n} task:{l_all} distance:{l_distance} normal: {l_normals} netwrench:{l_netwrench} rank: {l_rank} {torch.linalg.matrix_rank(grasp_matrix)} penetration: {l_penetration}")
                finger_dis = [state.contact_world_dist[self.box_contact_inds][finger_idx[i]].abs().min() for i in range(len(finger_idx))]
        ...
        path = os.path.join(os.path.dirname(self.obj_config.sdf_path), f'{self.count}_fca.obj')
        ...
        f = open(f'/apdcephfs/private_qihangfang/ibsdataset/{self.obj_name}', 'a')
        f.write(f'{self.nums} ')
        for i in range(7):
        ...
        f.write(f'{l_netwrench.item()} {l_distance_pure} {torch.linalg.matrix_rank(grasp_matrix).item()} {finger_dis[0].item()} {finger_dis[1].item()} {finger_dis[2].item()} {finger_dis[3].item()} {finger_dis[4].item()}\n')
        f.close()
