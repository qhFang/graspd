...
class FC():
    ...
    def run(self, initial_guess):
        ...
        for i in range(n):
            ...
            #l_distance = torch.mean(torch.exp(state.contact_world_dist[palm_idx] / 10))
            l_distance = torch.mean(state.contact_world_dist[palm_idx])
            
            net_wrench = state.contact_f_s[palm_idx]
            net_wrench[torch.isnan(net_wrench)] = 0
            net_wrench = torch.sum(net_wrench, 0)

            l_netwrench = torch.sqrt(torch.sum(net_wrench * net_wrench))

            grasp_matrix = state.contact_matrix[palm_idx].permute(1, 2, 0).reshape(6, -1)
            ...
            #l_all = 100.0 * l_rank + 0.0000001 * l_netwrench + l_distance + l_self_collision + l_joint
            l_all = l_distance
            hand_q_optimizer.zero_grad()
            l_all.backward()
            hand_q_optimizer.step()
            ...
            if i % record_every == 0:
                ...
                print(f"{i}/{n} task:{l_all} distance:{l_distance * 0.1} netwrench:{0.0000001 * l_netwrench} rank: {100.0 * l_rank} self_collision: {l_self_collision} joint: {l_joint}")
